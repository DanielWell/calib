...
int main()
{
	ifstream inImgPath("C:/Users/Administrator/MVS/Data/chess/chess.txt");
	...
	cv::Size pattern_size = cv::Size(8, 11);//标定板上每行、每列的角点数；测试图片中的标定板上内角点数为4*6
	...
	for (k = 0; k < image_num; k++)//遍历每一张图片
	{
		...
		for (i = 0; i < pattern_size.height; i++)
		{
			for (j = 0; j < pattern_size.width; j++)
			{
				...
				singleRealPoint.x = i * 10;//10 为 distance
